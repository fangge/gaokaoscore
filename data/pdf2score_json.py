...
def extract_category_from_filename(filename):
    # 例：10.舞蹈类总分分数段统计表（含本、专科层次加分）.pdf -> 舞蹈类
...
def extract_score_data_from_pdf(pdf_path):
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            # 假设表头包含"分数"或"人数"等关键词
            header = table[0]
            if not any("分数" in h or "人数" in h for h in header):
                continue
            score_idx = next((i for i, h in enumerate(header) if "分数" in h), None)
            count_idx = next((i for i, h in enumerate(header) if "人数" in h), None)
            cumu_idx = next((i for i, h in enumerate(header) if "累计" in h), None)
            if score_idx is None or count_idx is None or cumu_idx is None:
                continue
            score_data = []
            for row in table[1:]:
                try:
